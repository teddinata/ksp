...
Route::middleware(['jwt.auth', 'activity.log'])->group(function () {
    ...
    Route::prefix('salary-deductions')->group(function () {
            Route::get('/my-deductions', [App\Http\Controllers\Api\SalaryDeductionController::class , 'myDeductions'])
                ->middleware('role:admin,manager,anggota');
            // Read operations
...
Route::middleware(['jwt.auth'])->group(function () {
    ...
    Route::prefix('journals')->middleware('role:admin,manager')->group(function () {
            ...
            Route::get('/income-statement', [App\Http\Controllers\Api\JournalController::class , 'incomeStatement']);
            Route::get('/balance-sheet', [App\Http\Controllers\Api\JournalController::class , 'balanceSheet']);
            Route::get('/cash-flow', [App\Http\Controllers\Api\JournalController::class , 'cashFlow']);
...
Route::middleware(['jwt.auth'])->group(function () {
    Route::prefix('settings')->group(function () {
            ...
            Route::post('/{group}', [App\Http\Controllers\Api\SettingController::class , 'update'])
                ->middleware('role:admin,manager');
            Route::put('/{group}', [App\Http\Controllers\Api\SettingController::class , 'update'])
                ->middleware('role:admin,manager');
        }
        );
    });